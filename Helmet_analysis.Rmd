...
model01 <- lmer(stiffFrac ~ amp + freq + log(cycleNum) + chemIndex + porosity + 
                 (1|sampleNum), data = datano0)

model1 <- lmer(stiffFrac ~ chemIndex*log(cycleNum) + freq + amp+ porosity +  
                 (log(cycleNum)|sampleNum), data = datano0)
...
# Modeling dampFrac
```{r}
model02 <- lmer(dampFrac ~ porosity + chemIndex + freq + amp + log(cycleNum) + 
                 (1|sampleNum), data = datano0)

model2 <- lmer(dampFrac ~ porosity + chemIndex + freq + amp  + log(cycleNum) + 
