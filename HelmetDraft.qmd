---
title: "Helmet Report"
author: Armelle Duston, Emily Younger, Allison Comer
format: pdf
---
...
In this project, we analyze the results of an experiment designed to assess the behavior of different types of foam after many impacts. We are particularly interested in studying the behavior of  2 different porosities and 3 different chemical indexes. Each type of foam (which includes every combination of porosity and chemical index) underwent 10,000 or more oscillations, and measurements were taken at specified intervals to determine stiffness and damping. Data was collected for different amplitudes and frequencies of oscillations and at different initial strains. Our aim is to account for the effects of the conditions of the experiment in order to pick out the effects of porosity and/or chemical index on stiffness fraction and damping fraction over cycle count. These results will ultimately have important implications for the optimal foam used in military-grade helmets. 
...
#| fig-cap: "Correlation, distribution and scatterplots of continuous vairables"

sub <- subset(data, select = -c(sampleNum, cycleLet, damping, nonlinear, stiffness, porosity, chemIndex))

ggpairs(sub, progress = FALSE)+
  theme(axis.text = element_text(size = 8))

```


 As shown in the continuous variable correlation and distribution matrix above, stiffness fraction is highly correlated with frequency, amplitude, stresskpa and strain rate with values of -0.493, -0.559, 0.255 and -0.650. This shows that there is a strong negative relationship for stiffness fraction and frequency, amplitude and strain rate individually, then a weaker positive, linear relationship between stiffness fraction and stresskpa. 
...
ggplot(subset(data, cycleNum == 10000),
  ...
  geom_boxplot()+
  ggtitle("Damping Fraction at Cycle")
```
...
In this model, we use frequency, amplitude, chemical index, porosity, and log(cycle number) to predict stiffness fraction. The only variable which shows up as both a fixed effect and a random effect is log(cycle number), while all of the other predictors are purely fixed effects. We also include an interaction between chemical index and cycle number, which allows us to see that the relationship between chemical index and stiffness fraction is different depending on the cycle number. We investigated an interaction term between chemical index and porosity, since our EDA indicated that such an interaction may be present, but the terms were not significant and so we did not include it in our final model. 
...
In this model, the terms for porosity, frequency, amplitude, strain rate, and the log of the cycle number are significant. The terms for chemical index are not significant, but we chose to include these terms due to the client’s specific interest in this variable. The modeling assumptions are verified in the appendix. 
...
### Noise in Data
Given the noise present in the data due to an adjacent machine, this affected data may change the accuracy of our model. Specifically, since the adjacent machine was working in the later sample numbers, it creates a lack of randomness in the noise which in turn could create issues with equal variance and normality of residuals if persistent. 
